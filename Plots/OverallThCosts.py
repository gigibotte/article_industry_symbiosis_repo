...
bar_colors_boilers = ['#8E2999', '#8E2999', '#8E2999']  # Colors for boilers
bar_colors_district_heating = ['#D55E00', '#D55E00']     # Colors for district heating
...
for i, scenario in enumerate(scenarios, start=1):
    ...
    for location in locations:
        dfs[location] = pd.read_csv(os.path.join('data_article3', 'results', f"eco_results_{location}_df_th_{scenario}.csv"))
    ...
    for col, color in zip(legend_labels, bar_colors):
        fig.add_trace(
            go.Bar(
                ...
                name=col,
                marker_color=color,
                showlegend=True  # Only show legend in the first subplot
            ),
            row=1,
            col=i
        )
    ...
    for location in locations:
        ...
        fig.add_annotation(
            x=location,
            y=total_value * 1.2,  # Adjust the vertical position for better visibility
